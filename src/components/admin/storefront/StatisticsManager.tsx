  ...
  const handleSave = async () => {
    ...
    try {
      const { error: upsertError } = await supabase
        .from('settings')
        .upsert({
          key: 'storefront_statisticsmanager',
          value: settings
        });
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-bold text-[#3A2418]">Statistics</h3>
          <p className="text-sm text-[#5F5A54]">Manage the statistics counters section</p>
        </div>
        <button
          onClick={handleSave}
          disabled={isSaving}
      ...
      <div className="bg-[#FAF5EC] border border-[#E8DCC9] rounded-2xl p-6">
        <p className="text-sm text-[#5F5A54] mb-4">Settings for Statistics</p>
