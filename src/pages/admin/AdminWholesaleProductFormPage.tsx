...
const STEPS = [
  ...
  { id: 'details', label: 'Wholesale Product Details', icon: FileText, fields: ['short_description', 'description'] },
  { id: 'media', label: 'Media', icon: ImageIcon, fields: [] },
  { id: 'pricing', label: 'Pricing', icon: CreditCard, fields: ['regular_price', 'sale_price', 'cost_price', 'tax_class', 'is_wholesale', 'wholesale_price', 'wholesale_min_qty', 'wholesale_unit'] },
...
export default function AdminWholesaleProductFormPage() {
  const { id } = useParams();
  ...
  const fetchProduct = async () => {
    try {
      ...
      navigate('/admin/dashboard/wholesale');
    } finally {
      setLoading(false);
    }
  };
...
name: data.name,
      ...
      toast.success(isEditing ? 'Wholesale Product updated successfully' : 'Wholesale Product created successfully');
      navigate('/admin/dashboard/wholesale');
    } catch (error: any) {
