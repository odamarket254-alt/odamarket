...
const onAddressSubmitMatch = /const onAddressSubmit = async \([\s\S]*?\} catch \(error: any\) \{\s*toast\.error\(error\.message \|\| "Failed to create account\."\);\s*\} finally \{\s*setIsLoading\(false\);\s*\}\s*\};/;
...
code = code.replace(onAddressSubmitMatch, newOnAddressSubmit.trim());

const verifyOtpMatch = /const verifyOtp = async \(\) => \{[\s\S]*?\} catch \(error: any\) \{\s*console\.error\(error\);\s*toast\.error\(error\.message \|\| "Invalid or expired verification code\."\);\s*\} finally \{\s*setIsLoading\(false\);\s*\}\s*\};/;
...
code = code.replace(verifyOtpMatch, newVerifyOtp.trim());
