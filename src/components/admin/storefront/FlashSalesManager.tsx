  ...
  const fetchSettings = async () => {
    try {
      const { data, error } = await supabase
        ...
        .eq('key', 'storefront_flashsalesmanager')
        .single();
  ...
  const handleSave = async () => {
    ...
    try {
      const { error: upsertError } = await supabase
        .from('settings')
        .upsert({
          key: 'storefront_flashsalesmanager',
          value: settings
        });

      if (upsertError) throw upsertError;
      alert('Flash Sales updated successfully.');
    } catch (error: any) {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-bold text-[#3A2418]">Flash Sales</h3>
          <p className="text-sm text-[#5F5A54]">Manage flash sales and countdown timers</p>
        </div>
        <button
          onClick={handleSave}
          disabled={isSaving}
      ...
      <div className="bg-[#FAF5EC] border border-[#E8DCC9] rounded-2xl p-6">
        <p className="text-sm text-[#5F5A54] mb-4">Settings for Flash Sales</p>
