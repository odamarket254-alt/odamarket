  ...
  const fetchSettings = async () => {
    try {
      const { data, error } = await supabase
        .from('settings')
        .select('value')
        .eq('key', 'storefront_whychooseusmanager')
        .single();
  ...
  const handleSave = async () => {
    ...
    try {
      const { error: upsertError } = await supabase
        .from('settings')
        .upsert({
          key: 'storefront_whychooseusmanager',
          value: settings
        });
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-bold text-[#3A2418]">Why Choose Us</h3>
          <p className="text-sm text-[#5F5A54]">Manage feature cards for why customers should choose you</p>
        </div>
        <button
          onClick={handleSave}
          disabled={isSaving}
      ...
      <div className="bg-[#FAF5EC] border border-[#E8DCC9] rounded-2xl p-6">
        <p className="text-sm text-[#5F5A54] mb-4">Settings for Why Choose Us</p>
