...
-- ==============================================================
-- DATABASE SCHEMA MIGRATION FOR ODA MARKET
-- ==============================================================
...
CREATE TABLE IF NOT EXISTS public.profiles (
    ...
    email TEXT UNIQUE,
    phone TEXT,
    role TEXT DEFAULT 'customer' CHECK (role IN ('customer', 'supplier', 'admin', 'superadmin')),
    avatar_url TEXT,
...
-- Missing reviews table alias that code uses
...
-- ==============================================================
-- COMMUNICATIONS (NOTIFICATIONS, MESSAGES, TICKETS)
-- ==============================================================
...
CREATE TABLE IF NOT EXISTS public.ai_quotations (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
...
-- ==============================================================
-- ADD MISSING COLUMNS (Ensuring idempotency)
-- ==============================================================
DO $$
BEGIN
    -- profiles
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'verification_requested') THEN
        ALTER TABLE public.profiles ADD COLUMN verification_requested BOOLEAN DEFAULT false;
    END IF;
    ...
    -- rfq
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'rfqs' AND column_name = 'status') THEN
        ALTER TABLE public.rfqs ADD COLUMN status TEXT DEFAULT 'pending';
    END IF;
END $$;
...
-- Authenticated User Policies (Allow inserts/updates for admins or owners)
-- Profiles
CREATE POLICY "Users can view own profile" ON public.profiles FOR SELECT USING (auth.uid() = id);
CREATE POLICY "Users can update own profile" ON public.profiles FOR UPDATE USING (auth.uid() = id);
CREATE POLICY "Public can view profiles" ON public.profiles FOR SELECT USING (true); -- For supplier public pages
...
-- RFQs
CREATE POLICY "Users can view own RFQs" ON public.rfqs FOR SELECT USING (auth.uid() = buyer_id);
CREATE POLICY "Users can create own RFQs" ON public.rfqs FOR INSERT WITH CHECK (auth.uid() = buyer_id);
CREATE POLICY "Users can update own RFQs" ON public.rfqs FOR UPDATE USING (auth.uid() = buyer_id);

-- Admins can do everything
-- (Assuming simple policy, in reality you'd check auth.jwt() -> 'role' or profiles.role)
...
BEGIN
    FOR t IN 
        SELECT unnest(ARRAY['products', 'categories', 'orders', 'messages', 'notifications', 'settings', 'rfqs', 'inquiries', 'inquiry_messages'])
    LOOP
        EXECUTE format('
...
SELECT 
    ...
    (SELECT COUNT(*) FROM public.profiles WHERE role = 'supplier') AS total_suppliers,
    (SELECT COALESCE(SUM(total_amount), 0) FROM public.orders WHERE status = 'completed') AS total_revenue;

-- ==============================================================
-- STORAGE BUCKETS
-- ==============================================================
-- Note: Storage buckets and policies are typically managed via the Supabase Dashboard
-- or specific storage API calls. The following is conceptual SQL if using underlying schema.
