  ...
  const fetchSettings = async () => {
    try {
      const { data, error } = await supabase
        ...
        .eq('key', 'storefront_testimonialsmanager')
        .single();
  ...
  const handleSave = async () => {
    ...
    try {
      const { error: upsertError } = await supabase
        .from('settings')
        .upsert({
          key: 'storefront_testimonialsmanager',
          value: settings
        });

      if (upsertError) throw upsertError;
      alert('Customer Testimonials updated successfully.');
    } catch (error: any) {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-bold text-[#3A2418]">Customer Testimonials</h3>
          <p className="text-sm text-[#5F5A54]">Manage and approve customer reviews</p>
        </div>
        <button
          onClick={handleSave}
          disabled={isSaving}
      ...
      <div className="bg-[#FAF5EC] border border-[#E8DCC9] rounded-2xl p-6">
        <p className="text-sm text-[#5F5A54] mb-4">Settings for Customer Testimonials</p>
